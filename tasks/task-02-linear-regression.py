...
def plot_data(x, y, title="Generated Data"):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig('img/task-02-generated_data.png')
...
def plot_regression_line(x, y, model, title="Regression Line"):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig('img/task-02-regression_line.png')
def solve_linear_regression_nn(X_train, y_train, epochs=100, learning_rate=0.01):
    ...
    model = keras.Sequential([
        keras.layers.Dense(1, input_shape=(1,))  # Uma única camada com 1 neurônio
    ])
    
    # Compilação do modelo
    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=learning_rate),
        loss='mean_squared_error'  # MSE é o padrão para problemas de regressão
    )
    ...
    model.fit(
        ...
        y_train,
        epochs=epochs,
        verbose=0  # Não mostra progresso durante treinamento
    )
