...
class Perceptron:
    ...
    def activation(self, x):
        """Dado que estamos tratando de uma classificação binaria (-1 e 1), podemos usar a uma função de ativação que retorne um dos dois possiveis 
            valores apartir de um pivo, que definiremos como 0.
        """
        return np.where(x > 0, 1, -1)

    def predict(self, X):
        """ 
        Para realizar a predição, iremos utilizar o produto escalar entre os pesos e as features e aplicar a funcao de ativação.
        """
    ...
    def fit(self, X, y):
        """
        Para treinar o perceptron, iremos iterar sobre os exemplos de treinamento e atualizar os pesos de acordo com a funcao de ativação.
        """
        X_bias = np.c_[np.ones(X.shape[0]), X]
...
def main():
    ...
    plt.savefig("img/perceptron_output.png")
